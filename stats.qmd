...
### Chi-squared test
Chi-squared tests are used to determine whether one or more observed categorical variables follow expected distribution(s). For example, you may expect that 50% more movie goers attend movies on weekends in comparison to weekdays. After observing movie goers attendance for a month, you then can perform a chi-squared test to see if your initial hypothesis was correct. 

Chi-squared test can be used for two different purpose: 

- goodness of fit
- test for independence

### Chi-squared goodness of fit test
**Chi-squared (χ²) goodness of fit test** is a hypothesis test that determines whether an observed categorical variable with more than two possible levels follows an expected distribution. The null hypothesis (H0) of the test is that the categorical variable follows the expected distribution. The alternative hypothesis (Ha) is that the categorical variable does not follow the expected distribution. 
...
: Obversed number of website visitors
...
The H0 stated that the number of website visitors are the same for each weekday. As there are 3500 visitors in total, it means there should be 500 visitors each day. 

$$\chi^2 = \frac{(600-500)^2}{500} + \frac{(570-500)^2}{500} + ... + \frac{(490-500)^2}{500}$$
...
Now that you have calculated the 𝛸2 statistic, consider the following question: What is the probability of obtaining a 𝛸2 statistic of 97.6 or greater when examining 3,500 website visits if they null hypothesis is true? This is the question that the p-value—or “observed significance level"—will answer. 

A long time ago, Pearson realized that p-values for 𝛸2 statistics very closely corresponded with areas under certain curves, known as 𝛸2 curves. 𝛸2 curves represent probability density functions, and their shapes vary based on how many degrees of freedom are present in the experiment. Degrees of freedom are determined by the model, not by the data. This means that, in the website traffic example, the degrees of freedom are determined by how many different days a given visit can occur on—not by how many visits are sampled nor by the daily frequencies of the samples themselves. When the model is fully specified (i.e., you know all the possible categorical levels), then: 

$$\text{degrees of freedom} = \text{number of categorical levels} – 1$$
...
With seven categories, knowing the first six values lets us determine the seventh. For example, if you know the counts for Monday through Saturday, you can calculate Sunday’s count by subtracting the sum of those six days from the total (3,500). Therefore, only six values are "free" to vary independently.

The following figure depicts the 𝛸2 curves for three different degrees of freedom: five, 10, and 20.

![Chi-sqaured curves](images/chi_square_curves.png)

The p-value for a given 𝛸2 test statistic is very closely approximated by the area to its right beneath the 𝛸2 curve of the appropriate degrees of freedom. Notice that the more degrees of freedom there are in the experiment, the greater the area under the right tail of the curve for any given 𝛸2 test statistic, and therefore the greater the probability of getting a given 𝛸2 test statistic if the null hypothesis is true. 

The following figure contains the 𝛸2 curve for six degrees of freedom. For a 𝛸2 test statistic of, for instance, 10, the value of P is approximated by the shaded area under the curve where x ≥ 10. 

![Chi-squared curve with 6 degree of freedom](images/chi_sqare_curve_2.png)

In the case of the website visits, there are six degrees of freedom, but the 𝛸2 test statistic is 97.6—far along the x-axis in the right-skewed tail of the curve. The area under this interval is miniscule: 7.94e-19. In other words, the chances of getting a 𝛸2 test statistic ≥ 97.6 from 3,500 website visits if the null hypothesis were true are 7.94e-17%—practically zero. 

**Calculate the p-value with scipy**
...
# the degree of freedom is default at k - 1, with k = observed frequency
# you can adjust it with the parameter `ddof`, which is default to 0; adding a ddof of 1 would change the dof to: 
# k-1-ddof = k-1-1
result
```
...
**Step 2: Calculate the chi-squared test statistic (𝛘2)**

To calculate the 𝛸2 test statistic, arrange the data as a table that contains m x n values, where m and n are the number of possible levels contained within each respective categorical variable. The following table below breaks down the website visitors based on the device they used and their membership status.

| Observed values | Member | Guest | Total |
|-----------------|--------|-------|-------|
| Mac             | 850    | 450   | 1300  |
| PC              | 1300   | 900   | 2200  |
| Total           | 2150   | 1350  | 3500  |

: Website visitors based on the device they used and their membership status. 

Notice that the table starts with 2 x 2 known values (two levels for each category), from which totals are derived. These totals can be used to calculate the expected values, which are necessary to get the 𝛸2 test statistic. 
...
$$\text{expected value} = \frac{2150-1300}{3500}$$
...
Let's use the example of students studying for an exam. Suppose you wanted to compare three different studying programs, A, B, and C to determine whether they have an effect on exam score. Here is the data: 
...
df = pd.DataFrame(
    {
        ...
        'Study Program': ['A', 'A', 'A', 'A', 'B', 'B', 'B', 'B', 'C', 'C', 'C', 'C'],
        'Exam Score': [88, 79, 86, 90, 94, 84, 87, 89, 85, 76, 81, 78]
        }
        )
...
# df.groupby('Study Program')['Exam Score'].mean()
```

**Step 1: determine the hypothesis & confidence level**
...
The grand mean is the overall mean of all samples in all groups.
...
$$\text{SSB} = \sum_{g=1} n_g (M_g - M_G)^2$$

where
​	
$n_g$ = the number of samples in the gth group
$M_g$ = mean of the gth group	
$M_G$  = grand mean

$$SSB = 4(85.75-84.75)^2 + 4(88.5-84.75)^2 + 4(80-84.75)^2$$

$$SSB = 150.5$$

**Step 4: Calculate the sum of squares within groups (SSW)**

$$\text{SSW} = \sum_{g=1} \sum_{i=1} (x_{gi} - M_g)^2$$

where:
$x_{gi}$= sample i of the gth group
$M_g$= mean of the gth group
...
The following script uses the student exam score as an example to demonstrate the steps of calculation for SSW: 
...
group_mean = df.groupby('Study Program')['Exam Score'].mean().reset_index()

group_mean = group_mean.rename(columns={'Exam Score': 'Group Mean Score'})

df = df.merge(group_mean, 'left', on='Study Program')

df['sq_in_gp'] = (df['Exam Score'] - df['Group Mean Score'])**2

# SSW = sum(df.groupby('Study Program')['sq_in_gp'].sum())
...
**Step 5: Calculate the mean square between and within group**
...
why $n-k$: 
$$(n_1 - 1) + (n_2 - 1) + (n_3 - 1) = (n_1 + n_2 + n_3) - 3 = n - k$$
...
**Step 7: Get p-value**
 ...
 Now you can use F-distribution and the F-statistic to get a p-value, which you use to decide whether to reject the null hypothesis. 

 Similar to t-tests and 𝛸2 tests, ANOVA testing finds the area under a particular probability distribution curve—the F-distribution—of the null hypothesis to determine a p-value. The larger the F-statistic, the lesser the area beneath the curve and the more evidence against the null hypothesis, thus resulting in a lower p-value. 
...
![F-distribution for multiple degree of freedom](images/f_distribution.png)

Similar to 𝛸2 curves, F-distributions help determine the probability of falsely rejecting the null hypothesis. In the case of ANOVA, this probability is represented by the area of the F-distribution beneath the curve where x ≥ your F-statistic. For example, the following graph depicts the F-distribution for the exam scores example. It has two degrees of freedom in the numerator ($k-1$) and nine degrees of freedom ($n-k$)in the denominator. The area beneath the curve where x ≥ 4.04 (the computed F-statistic) is shaded.

![F-distribution for the exam score example](images/f_distribution_2.png)

In this case, the area beneath the F-distribution to the right of 4.04 is 0.05604. This is the probability of observing an F-statistic greater than 4.04 if the null hypothesis were true. Whether this is sufficient to reject the null hypothesis is a decision you make at the beginning of your hypothesis test. For example, if you decided that you wanted a confidence level of 95% or greater, you cannot reject the null hypothesis that the means of the distributions for each program of study are all the same, because the p-value is 0.056.

### Type I, II & III ANOVA
Not to be confused with one-way or two-way ANOVA, the type of ANOVA is a distinct concept. It refers to how the sums of squares (these quantities are the building blocks for ANOVA) are calculated, while "K-way" means that there are K categorical factors in the analysis. 

For one-way ANOVA, all types give identical results, so the choice doesn’t matter. 

For two-way/multi-factor ANOVA, consider: 

- Use Type I if there’s a logical order or hierarchy in factor entry.
- Use Type II for balanced data and when there’s no interaction.
- Use Type III for unbalanced data or if interactions exist.

### ANOVA in Python
In `python`, you can use the `statsmodel` package to build a linear regression model, then use the `anova_lm` function to calculate the f-statistics. 
...
# Run one-way ANOVA
sm.stats.anova_lm(model, typ = 2) # typ argument refers to the three types of ANOVA tests
```
...
In python, you can use the `pairwise_tukeyhsd` function from `statsmodels` to perform the test. 

```{{python}}
from statsmodels.stats.multicomp import pairwise_tukeyhsd

tukey_oneway = pairwise_tukeyhsd(endog=data['Sales'], # the Y variable
                                 groups=data['TV'], # the categorical X
                                 alpha=0.05) # this set the confidence level of all tests to 95%

tukey_oneway.summary()
```

The above codes will return a summary table for the post-hoc test. The reject column indicate whether if we reject the null hypothesis for each pair comparison. In this case, null hypothesis for all comparisions are rejected. 


![Post-hoc test summary in python. ](images/posthoc_test_summary.png)
...
The logistic function is used in logistic regression to model the probability p(x). Specifically:

- $\beta_0$ is the intercept of the model.
- $\beta_1$ is the coefficient associated with the feature x. 

The logistic function maps any input x to a value between 0 and 1, ensuring the output is a valid probability. 
